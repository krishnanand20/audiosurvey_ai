...
def run_once():
    from app.twilio_handler import call_from_csv
    log("Scheduler run_once started")
    call_from_csv()
    log("Scheduler run_once finished")

def run_loop(interval_sec: int = 300):
    log(f"Scheduler run_loop started (interval_sec={interval_sec})")
    while True:
        try:
            run_once()
...
def start_scheduler_in_background(interval_sec: int = 300) -> None:
    """
...
def call_status():
    ...
    # If Twilio says completed but participant never spoke, keep pending for retry
...
if __name__ == "__main__":
    ...
    if mode == "call":
        ...
        app.run(host="0.0.0.0", port=5050, debug=False)# app/twilio_handler.py
...
def start_scheduler_in_background(interval_sec: int = 300) -> None:
    ...
    This version does NOT require app.scheduler.run_loop to exist.
    It will call app.scheduler.run_once() every interval_sec seconds.
    """
    def _loop():
        # Import inside thread to avoid circular imports at module load time
        from app import scheduler
        while True:
            try:
                scheduler.run_once()
            except Exception as e:
                print(f"[scheduler-thread] ERROR: {repr(e)}")
            time.sleep(interval_sec)
...
if __name__ == "__main__":
    ...
    if mode == "call":
        ...
        # ✅ Scheduler runs automatically in background while server is running
